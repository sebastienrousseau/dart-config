...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/dart-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Strict Dart & Flutter Analysis Rules](#strict-dart-flutter-analysis-rules) — before and after rule comparison
...
Install `@sebastienrousseau/dart-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/dart-config

# pnpm
pnpm add -D @sebastienrousseau/dart-config

# yarn
yarn add -D @sebastienrousseau/dart-config

# bun
bun add -d @sebastienrousseau/dart-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/dart-config.git
cd dart-config
make            # check + test
```
...
- **Target environment**: Optimized for Dart 3.4+ / Flutter.
...
```bash
cp node_modules/@sebastienrousseau/dart-config/analysis_options.yaml ./analysis_options.yaml
```
...
`@sebastienrousseau/dart-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/dart-config` solves this with three deliberate engineering choices:
...
## Strict Dart & Flutter Analysis Rules

Enforces sound null safety, exhaustive switch pattern matching, strict casts, and Flutter architectural best practices.
...
### After (@sebastienrousseau/dart-config Enforced)

```text
# analysis_options.yaml
include: package:@sebastienrousseau/dart-config/analysis_options.yaml
analyzer:
  language:
    strict-casts: true
...
When working on legacy Dart 1.x or pre-null-safety Dart 2.x projects.
